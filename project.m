...
for index = file_process_range
    signal_out = process_audio(filenames(index), boundary_arr, central_freq_arr, desired_Fs, band_gains); % call processing function
    % save_audio(filenames(index), iteration_str, signal_out, desired_Fs); % save to output folder
    soundsc(signal_out(1:100000), desired_Fs); % play output to speakers
end
